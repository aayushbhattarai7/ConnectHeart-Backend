...
class AuthService {
  ...
  bcryptService
  constructor(
    getDet = database_config_1.AppDataSource.getRepository(details_entities_1.userDetails),
    getDetails = database_config_1.AppDataSource.getRepository(auth_entity_1.Auth),
    bcryptService = new bcrypt_utils_1.default()
  ) {
    this.getDet = getDet
  ...
  async login(data) {
    try {
      ...
      if (!user) throw HttpException_utils_1.default.notFound(message_1.Message.invalidCredentials)
