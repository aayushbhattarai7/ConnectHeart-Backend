...
const User = () => {
    ...
    return (
        <div>
             <div className=" flex justify-center"><h1 className="mr-64 font-poppins font-medium">people may you know</h1>
             </div>
            <div className="justify-start flex flex-wrap gap-8 overflow-hidden">
        {error && <p>{error}</p>}
        {users?.map(user => (
            
          <div key={user?.id}  className=" flex flex-col p-5 mb-10 justify-center items-center w-64 shadow-xl">
            <div onClick={() => handleUserClick(user.id)}>
            {user?.profile?.path ? (
                <img className="h-44 w-44 rounded-2xl mb-3" src={user?.profile?.path} alt="Profile" />
            ) : (
                ...
                <button className="border border-blue-400 bg-blue-600 text-white rounded-lg w-40" onClick={() =>sendRequest(user?.id)}>Send</button>
            </div>
        ))}
        </div>
