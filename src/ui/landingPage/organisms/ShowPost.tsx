...
const ShowPost = () => {
    ...
    const renderComments = (comments: Comment[], isChild: boolean = false) => {
        ...
        return (comments.map(cmt => (
            ...
            <div key={cmt?.id} className='flex justify-start items-center'>
                <div key={cmt.id} className={`relative mb-4 ${isChild ? 'ml-7' : 'ml-4'}
                    ...
                    <div className=''>
                        <div className='flex gap-2 p-2'>
                            {cmt?.commentAuth?.profile?.path ? (
                                <img className="w-8 h-8  rounded-full" src={cmt?.commentAuth?.profile?.path} alt="Profile" />
                            ) : (
                                <img className="w-8 h-8  rounded-full" src="/profilenull.jpg" alt="Default Profile" />
                            )}
                            <p className='mt-1'>{cmt?.commentAuth?.details?.first_name}  {cmt?.commentAuth?.details?.last_name} </p>
                        </div>
                        ...
                        <div className='' key={id}>
                            ...
                            {isVideo && (
                                <video className='rounded-lg' controls>
                                    <source className='rounded-2xl' src={path} type={`video/${path?.split('.').pop()}`} />
                                </video>
                            )}
                        </div>
                    )
    ...
    return (
        <div className='mt-20 flex flex-col '>
            <div className='flex flex-col justify-start items-center overflow-y-auto h-screen mt-30  w-maxx ml-72 
            shadow-xl   p-10 mb-16 bg-gray-100' style={{
                    msOverflowStyle: 'none',
                    scrollbarWidth: 'none'
                }}>

                <div className='flex justify-end items-start'>
                    <Post postId={posts[0]?.id || ''}
                        refresh={getPost} />
                </div>
                ...
                {posts.map(post => (
                    ...
                    <div className='shadow-xl p-10 mb-16 text-ellipsis bg-white' key={post.id}>
                        <div key={post.postIt?.id} className='bg-white  mb-5' onClick={() => handleUserClick(post?.postIt?.id)}>

                            <div className="flex bg-white  p-4">
                                {post?.postIt?.profile?.path ? (
                                    <img className="w-16 h-16  rounded-full " src={post?.postIt?.profile?.path} alt="Profile" />
                                ) : (
                                    <img className="w-16 h-16  rounded-full " src="/profilenull.jpg" alt="" />
                                )}

                                <div className='flex gap-1 p-5 mb-3 ' onClick={() => handleUserClick(post?.postIt?.id)}>
                                    <p className='font-medium font-poppins text-lg'>{post.postIt?.details?.first_name}</p>
                                    <p className='font-medium font-poppins text-lg'>{post.postIt?.details?.last_name} </p>
                                    {post?.feeling ? (
                                        <p className='font-poppins'> is feeling <span className='font-poppins font-medium'>{post?.feeling}</span> </p>
                                    ) : null}
                                </div>
                            </div>

                            <div className='border mb-3 max-w-[45rem] p-2 rounded-2xl break-words'>
                                <div className='flex flex-col justify-start items-start mb-4 rounded-lg' key={post.id} >
                        ...
                        <div className='flex gap-10'>
                            ...
                            {post?.likes?.map((like) => (
                                like?.auth?.id && like?.isLiked && (
                                    <p key={like.auth.id}>{like.auth.id}</p>
                                )
                            ))}



                            <button className='rounded-xl bg-blue-700 h-7 pl-5 text-white w-14' onClick={showComment}>
