...
const MessageUser = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('read', ({ read, senderId }) => {
      if (read) {
  ...
  const unreadchat = async (id?: string) => {
    try {
      ...
      console.log(response, 'ajdhuahduaudh');
    } catch (error) { 
      ...
      <div className=" ml-[18rem] mt-24 justify-start fixed top-0 w-[39rem] bg-gray-100  p-8 items-start h-screen border">
        <div className="justify-start flex flex-col gap-2 mb-2 overflow-hidden">
          ...
          {connects?.map((connect) => {
                const unreadCount = chats.filter((chat) => chat.sender.id === connect.id && !chat.read).length
            return(
               <div
              key={connect?.id}
            ...
            >
              <div className="" onClick={() => unreadchat(connect?.id)}>
                ...
                <div className="flex pt-2">
                  ...
                  <div className="flex justify-center w-[20rem] pl-3 items-center">
                    <div className="mb-3 mt-2 w-[10rem] mr-16 flex flex-col font-medium">
                      ...
                      {unreadCount > 0 && (
                  <span className='bg-red-500 text-white rounded-full text-xs w-5 py-[2px] px-[6px] h-5'>{unreadCount}</span>
                )}
                    </div>
                    <p>5:30 pm</p>
                  </div>
