...
class AuthService {
  ...
  async create(image: any, data: AuthDTO): Promise<Auth> {
    try {
      ...
      if (image) {
        ...
        if (allowedMimeTypes.includes(image?.mimetype)) {
          ...
          throw HttpException.badRequest(
            'Invalid file type. Only jpg, jpeg, and png are accepted.',
          );
        }
      }
      ...
      await this.mailService.sendMail({
        ...
        html: generateHtml(
          `Hey ${details.first_name}! Welcome to the ConnectHub`,
        ),
      });
      return auth;
    } catch (error: any) {
  ...
  async passwordReset(userId: string, data: ResetPasswordDTO): Promise<string> {
    try {
      ...
      console.log(
        '🚀 ~ AuthService ~ passwordReset ~ data.password:',
        data.password,
      );
  ...
  async updateUser(userId: string, image: any, data: AuthDTO): Promise<Auth> {
    try {
      ...
      auth.details.phone_number =
        data?.phone_number || auth.details.phone_number;
